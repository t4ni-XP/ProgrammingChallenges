...
for j in range(int(number) + 1):
    ...
    if num_len == 1:
        ...
        comp_str = str_num[1:num_len]
        if str_num[0] == str_num[1]:
            if comp_str in dp_up:
                count += 1
                dp_up.add(str_num)
            elif comp_str in dp_down:
                count += 1
                dp_down.add(str_num)
            elif comp_str in dp_eq:
                count += 1
                dp_eq.add(str_num)
        elif str_num[0] < str_num[1]:
            if comp_str in dp_up:
...
if number in dp_up:
    print(count-1)
elif number in dp_down:
    print(count - 1)
elif number in dp_eq:
    print(count - 1)
else:
    print(-1)
